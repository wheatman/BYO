...
sequence<edge_entry> fetch_intercluster_small(Graph& GA, C& clusters,
  ...
  parallel_for(0, n, 1,
               [&](size_t i) { GA.map_out_neighbors(i, map_f); });
...
sequence<edge_entry> fetch_intercluster_te(Graph& GA, C& clusters,
  ...
  parallel_for(0, n, 1, [&](size_t i) {
    deg_map[i] = GA.count_out_neighbors(i, pred);
  });
  deg_map[n] = 0;
...
// TODO(wheatman) should the user always have to specify the out graph format
template <class graph,
          class out_Graph = Graph<gbbs::graph_implementations::symmetric_graph<
                                      gbbs::symmetric_vertex, gbbs::empty>,
                                  true>>
inline auto contract(graph &GA, sequence<uintE> &clusters,
  ...
  auto GC = sym_graph_from_edges<out_Graph>(/* edges = */ sym_edges,
                                              /* n = */ num_ns_clusters);
