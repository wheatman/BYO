...
#define generate_coo_main(APP, mutates)                                        \
  int main(int argc, char* argv[]) {                                           \
    ...
    if (compressed) {                                                          \
      ...
      if (symmetric) {                                                         \
        auto G = gbbs::gbbs_io::read_unweighted_symmetric_graph<unweighted_sym_graph>(iFile, mmap,   \
                                                                binary);       \
        auto G_coo = to_edge_array<gbbs::empty>(G);                            \
        run_app(G_coo, APP, mutates, rounds)                                   \
      } else {                                                                 \
        auto G = gbbs::gbbs_io::read_unweighted_asymmetric_graph<unweighted_asym_graph>(iFile, mmap,  \
                                                                 binary);      \
        auto G_coo = to_edge_array<gbbs::empty>(G);                            \
        run_app(G_coo, APP, mutates, rounds)                                   \
      }                                                                        \
...
#define generate_symmetric_main(APP, mutates)                                  \
  int main(int argc, char* argv[]) {                                           \
    ...
    if (compressed) {                                                          \
      ...
      auto G =                                                                 \
          gbbs::gbbs_io::read_unweighted_symmetric_graph<unweighted_sym_graph>(iFile, mmap, binary); \
      run_app(G, APP, mutates, rounds)                                         \
    }                                                                          \
  }
