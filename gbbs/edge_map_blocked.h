...
inline vertexSubsetData<data> edgeMapSparse(Graph& G, VS& indices, F& f,
  ...
  if (should_output(fl)) {
    ...
    parallel_for(
          ...
          if (fl & in_edges) {
            G.parallel_map_in_neighbors(
                v, [&](auto src, auto target, auto weight) {
                  if (f.cond(src)) {
                    auto m = f.updateAtomic(v, src, weight);
                    if constexpr(std::is_same_v<data, gbbs::empty>) {
                      if (m) {
                        outEdges.push_back(src);
                      }
                    } else {
                      if (m.has_value()) {
                        outEdges.push_back({src, *m});
                      }
                    }
                  }
                });
        ...
        1);
    auto nextIndices = outEdges.get_sequence();
      ...
      [&](size_t i) {
        ...
        if (fl & in_edges) {
          ...
          G.parallel_map_out_neighbors(
              v, [&](auto src, auto target, auto weight) {
                if (f.cond(target)) {
                  f.updateAtomic(v, target, weight);
                }
              });
        }
      },
...
inline vertexSubsetData<data> edgeMapChunked(Graph& G, VS& indices, F& f,
                                             const flags fl) {
  return edgeMapSparse<data>(G, indices, f, fl);
  // if (fl & no_output) {
  //   return edgeMapSparseNoOutput<data, Graph, VS, F>(G, indices, f, fl);
  // }
  // using S = typename vertexSubsetData<data>::S;
